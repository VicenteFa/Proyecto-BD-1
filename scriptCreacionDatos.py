...
FECHA_INICIO = date.today() - timedelta(days=3 * 365) # 3 años atrás 
DIAS_ENTRE_COMPRAS = 15 # Aprox 2 compras al mes 

def generar_datos():
    print("Iniciando generación de datos...")
    
    # 1. Fabricantes (10 por país, 5 países) 
    fabricantes_list = []
    ...
    # 2. Cigarrillos (10 tipos por fabricante) 
    cigarrillos = []
    with open('cigarrillos.csv', 'w', newline='', encoding='utf-8') as f:
        ...
        for fab_nombre, pais in fabricantes_list:
            for m in MARCAS_BASE:
                ...
                writer.writerow([marca, filtro, color, clase, mentol, round(random.uniform(0.1, 1.5), 2), round(random.uniform(1.0, 14.0), 2), fab_nombre, 6.5, 2.0, 10, 20])

    # 3. Estancos (15 por localidad, 10-15 localidades/provincia) 
    estancos = []
    ...
    # 4. Almacenes, Compras y Ventas (3 años de info) 
    with open('almacenes.csv', 'w', newline='', encoding='utf-8') as f_alm, \
        ...
        for nif in estancos:
            # 10-30 almacenes por estanco 
            for cig in random.sample(cigarrillos, random.randint(10, 30)):
                w_alm.writerow([nif] + list(cig[:5]) + [random.randint(50, 500)])
                
                for i in range(72): # 72 periodos (3 años * 24 registros)
                    fecha = FECHA_INICIO + timedelta(days=i * DIAS_ENTRE_COMPRAS)
                    w_comp.writerow([nif] + list(cig[:5]) + [fecha, random.randint(10, 50), 20.0])
                    w_vent.writerow([nif] + list(cig[:5]) + [fecha + timedelta(days=5), random.randint(1, 8), 6.5])
